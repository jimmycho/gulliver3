...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동 
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='./admin_bookinfoList?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>이전</A></span>"); 
    } 
    ...
    for(int i=startPage; i<=endPage; i++){ 
      ...
      if (nowPage == i){ 
        ...
        str.append("<span class='span_box_1'><A href='./admin_bookinfoList?col="+col+"&word="+word+"&nowPage="+i+"'>"+i+"</A></span>");   
      } 
    } 
     
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='./admin_bookinfoList?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
